<!-- 보관(2026-09-01): 피그마와 정본 대조용. 화면 HTML 검사와 성격이 달라 분리해 보관. -->
---
name: design-qa-figma-DEPRECATED
description: 【보관】 피그마 REST 덤프로 피그마 파일과 정본을 대조하던 옛 design-qa. 화면 HTML 검사(새 design-qa)와 성격이 달라 분리 보관. 호출하지 않는다. 원래 description: 피그마 파일이 시디즈 디자인 시스템 정본(design-system/)을 지키는지 검사하고, 위반 항목을 컴포넌트→variant→레이어 단위로 보고한다. "디자인 QA", "피그마 정본 검사", "design-qa", "디자인 시스템 위반 검사", "타이포/컬러 규칙 지키는지 확인" 류 요청에 사용. REST 덤프 → 타이포·컬러·컴포넌트·스페이싱·네이밍 검사 → 심각도별 보고 → audits/ 저장. 피그마는 읽기만 하고 수정하지 않는다.
---
...
- **⚠️ 절대 원칙(예외 없음):** 정본은 **컬러 15색(Blue 3 / Red 2 / Grey 10) + 타이포 14종**뿐이다. 이 밖의 색·크기·굵기는 **오류이며 "위반"으로 분류**한다. **근사값이라도 예외·"주의" 처리하지 않는다** — 가장 가까운 정본 토큰을 권장값으로 병기한다. 신규·중간·근사 토큰 제안 금지(팔레트 확장 방향 금지). 유일한 통과 대상은 아래 ①문서/가이드 색 ②로고 자산 색 ③컴포넌트 로컬 확장색(색+지정 범위) 세 allowlist뿐이다.
...
- 팔레트(Blue-700/500/100, Red-600/100, Grey-900~50) 외 hex 직접 입력 → **위반**
- 팔레트와 근사하지만 다른 값 사용(예: `#FEFEFE` vs Grey-50 `#FFFFFF`, `#ECECEC` vs Grey-200, `#B3B3B3` vs Grey-400) → **근사값도 예외 없이 "위반"**. 가장 가까운 정본 토큰을 권장값으로 병기(주의로 강등 금지).
...
- **심각도별 분류:**
  - **위반** — 정본과 어긋남. **정본 15색·14종 밖 색·폰트는 근사값이라도 전부 위반**(T1·T2 전부, T3·T4 명백 케이스). 3종 allowlist(문서/가이드 색·로고 자산 색·컴포넌트 로컬 확장색)만 통과.
  - **주의** — 색·폰트가 아닌 판단 애매 케이스만(인스턴스 오버라이드 등). **근사 컬러를 주의로 강등하지 않는다.**
  - **정보** — 네이밍 등 (T5)
