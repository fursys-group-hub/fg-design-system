...
- **구성(순서 고정):**
  ...
  2. **경로 A. 클로드 챗** — 회사 공유 프로젝트 링크 `{PROJECT_LINK}`로 접속해 바로 사용. 프로젝트에 디자인 시스템이 **이미 세팅**되어 있음을 안내.
  ...
  6. **문의** — 관리자 연락처 `{CONTACT}`.
- **플레이스홀더:** `{PROJECT_LINK}`, `{CONTACT}`는 **문자 그대로** 남긴다(추후 관리자가 채움).
