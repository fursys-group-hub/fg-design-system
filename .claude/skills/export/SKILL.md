...
- **구성(순서 고정):**
  1. **시작** — 이 문서가 무엇인지 + **버전 확인 방법**(최상단 스탬프의 날짜·커밋을 저장소 최신과 대조) 2~3줄.
  2. **경로 A. 클로드 챗** — 회사 공유 프로젝트 링크 `https://claude.ai/project/019ff881-be01-75f1-921a-72c7db7b4f8e`로 접속해 바로 사용. 프로젝트에 디자인 시스템이 **이미 세팅**되어 있음을 안내.
  3. **경로 B. 클로드 코드/터미널** — 저장소 클론(`git clone`), `git pull`로 최신화, `/sidiz-design-system`·`/design-qa` 스킬 사용법 간단 안내.
  4. **경로 C. Cowork** — 이 `dist/sidiz/` 폴더를 작업 폴더로 지정(CLAUDE.md 자동 적용). *(환경 3종 중 세 번째)*
  5. **공통 규칙** — 정본 토큰만 사용 · 문서 **직접 수정 금지**(수정 요청은 디자인 시스템 관리자에게) · 컴포넌트 로컬 색은 문서화된 범위에서만.
