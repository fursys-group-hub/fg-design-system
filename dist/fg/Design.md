...
컴포넌트 변수 바인딩 값에서 집계한 실사용 스케일. **값은 확정이며, 코드 명칭 `--fg-space-{n}` · `--fg-radius-{n}` 이 기준이다.**

스페이싱과 Radius 변수는 피그마에 등록되어 있지 않다. 피그마의 로컬 발행 변수는 색뿐이다(Fursys Group 22개 · Secondary Colors 65개). 따라서 대조할 원장 명칭이 없고, 코드 명칭 `--fg-space-{n}` · `--fg-radius-{n}` 이 기준이다. 값은 정본 스케일과 일치한다. (2026-09-04 내보낸 피그마 파일 기준으로 확인)
...
스페이싱은 **세 층**으로 나뉜다(레이아웃 여백 · 컴포넌트 내부 미세값 · 모서리 반경). 자세한 층 구분·쓰임은 정본 `design-system/tokens/spacing.md`.

**① 레이아웃 여백 (`--fg-space-{n}`)** — 화면 제작 시 고르는 값. **4 또는 8의 배수에서 고른다.**
`4 · 8 · 12 · 16 · 20 · 24 · 32 · 40 · 64 · 80 · 100` (px). 주력 **8·12**, 큰 값(64·80·100)은 섹션/레이아웃 여백.

**② 컴포넌트 내부 미세값 (`--fg-space-2 · -5 · -6 · -10`)** — 정본 컴포넌트에 박혀 있는 값. 마크업 복사 시 따라오며 **새로 만들어 쓰지 않는다.** `gap 2`(사이드바 메뉴·프로필 이름·드롭다운), `padding-bottom 5`(카드 단위 밑선), `gap 6`(라벨-입력·flat/text 버튼 아이콘), `10`(flat 버튼 좌우·드롭다운 행).

### Corner Radius (`--fg-radius-{n}`) — 모서리 반경 (여백 아님)
`2 · 4 · 5 · 6 · 32 · 40 · 9999` (px)
- 버튼/인풋 기본 **4·6**, 큰 곡률 32·40, `9999`=pill(태그/칩/원형).

> **집계 빈도 주의:** 노드 사용 빈도 숫자는 `padding`·`gap` 외에 같은 px 값의 `radius`·`outline`·`border` 까지 포함한 것이다. 빈도가 층 구분과 1:1로 맞지 않는다.
