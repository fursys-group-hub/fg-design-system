<!-- 퍼시스그룹 디자인 시스템 작업 지침 — 2026-08-21 전면 개정 (v5) -->
...
**시스템명 앞에는 담당자가 고른 한글 브랜드명을 붙여 적는다.** 브랜드명과 시스템명 사이는 공백 한 칸이고 괄호는 쓰지 않는다. 예: `시디즈 수요예측 시스템`, `일룸 재고관리 시스템`. 그룹사 공통(`group`)을 고른 경우에는 `퍼시스그룹` 을 붙인다.
...
| 4 | 페이지네이션 | `.fg-pagination__nums > button` 숫자 / 좌우 화살표 | 숫자=현재 페이지 지정. 화살표=앞뒤 한 칸 이동(첫·끝에서 비활성) |
...
- 링크는 `<a href>` 로 만든다. `<span>` 으로 만들지 않는다. `<span>` 은 키보드 Tab 으로 이동할 수 없어 포커스 표시가 나타나지 않는다.
- 테이블은 컨테이너 폭에 맞춰 늘어난다. 콘텐츠 폭 기준은 1280.
- 페이지네이션은 **테이블 가로 폭 기준 중앙 정렬**이다. `fg-pagination-wrap` 으로 감싼다.
- 페이지네이션 숫자와 좌우 화살표는 `<button>` 으로 만든다. `<span>` 이나 `<i>` 태그만으로 만들지 않는다. 그렇게 만들면 키보드 Tab 으로 이동할 수 없어 포커스 표시가 나타나지 않는다.
...
키보드로 이동할 때 지금 어디에 있는지 보이게 한다.

- 포커스 표시는 outline 으로 그린다. 요소 위치가 밀리지 않는다.
- 색은 Primary-600, 두께 2px, 요소에서 2px 띄운다.
- :focus-visible 을 쓴다. 마우스로 눌렀을 때는 나타나지 않고
  키보드로 이동했을 때만 나타난다.
- 대상은 버튼, 인풋, 탭, 체크박스, 라디오, 드롭다운,
  링크, 페이지네이션이다.
- outline: 0 으로 포커스 표시를 지우지 않는다.
