...
- **반응형 브레이크포인트는 `max-width: 768px` 하나다.** 768 이하가 태블릿 세로와 모바일을 함께 다룬다 — 구간을 쪼개면 관리 지점만 늘고 한쪽이 뒤처지므로 새 브레이크포인트를 추가하지 않는다.
- **고객 화면 모바일 기준 폭은 375px 이다**(`CLAUDE-customer.md` 「폭과 마진」의 값과 같다). 모바일 축소는 `fg-customer.css` 의 `@media (max-width: 768px)` 한 곳에서만 한다 — 정의 지점을 하나로 둬야 어긋나지 않는다.
- **업무 화면은 반응형을 지원하지 않는다. 데스크톱 전용이다.** 사내 관리 화면이라 데스크톱에서만 쓰기 때문이다.
