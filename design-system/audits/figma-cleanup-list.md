...
## 상태 갱신 (2026-08-20 원본 재검증 기준)

### 원인 규명 (교정 방법 변경)
정본 밖 색은 **하드코딩이 아니라 외부 라이브러리 변수 바인딩**이었다. 매핑:
- 외부 `Gray-600` = `#B3B3B3`
- 외부 `Gray-400` = `#ECECEC`
- 외부 `Gray-900` = `#1A1A1A`
- 외부 `Red-200` = `#EF2E32`

→ 교정 방법을 **"값 치환"에서 "로컬 Collection 1 변수로 재바인딩"으로 변경.** (원시 hex를 바꾸는 게 아니라 외부 변수 참조를 로컬 정본 변수로 다시 연결한다.)
...
| 컴포넌트 | 레이어 | 노드 ID | 재바인딩 |
|---|---|---|---|
| Table Cell | 번호 | `66:1797` | → Grey-400 |
| Table Cell | Line 1 | `66:1799` | → Grey-200 |
| Sidebar | 우측 보더 | `69:5383` | → Grey-200 (fill도 Grey-50 재바인딩 권장) |
| Sidebar | 우측 보더 | `69:5419` | → Grey-200 (fill도 Grey-50 재바인딩 권장) |
| Sidebar | 우측 보더 | `69:5464` | → Grey-200 (fill도 Grey-50 재바인딩 권장) |
| Sidebar | 우측 보더 | `82:9521` | → Grey-200 (fill도 Grey-50 재바인딩 권장) |

### ⛔ 미해결 (숨김)
- Carousel `Rectangle 3962` **2건**
- Stepper `Frame 2285`·`Vector 3` **6건**
- Stepper 화살표 아이콘 **7건**
- Input Case `CircleAlert` **1건**

> 아래 A~D는 2026-08-20 재검증 이전의 초기 추출 기록(참고용). 최신 상태는 위 "상태 갱신"을 따른다.
